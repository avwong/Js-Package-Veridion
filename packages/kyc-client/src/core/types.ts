...
export interface InitOptions {
  ...
  // Future-friendly options (not used in insecure mode, but scaffolded)
...
export type KycResult =
  | { ok: true; status: KycStatus; token?: string; expiresAt?: number }
...
export type EdgeMessage =
  | { type: "EDGE_READY" }
  | { type: "EDGE_COMPLETE"; token: string; status: KycStatus }
  | { type: "EDGE_CANCEL" }
  | { type: "EDGE_ERROR"; error: string };

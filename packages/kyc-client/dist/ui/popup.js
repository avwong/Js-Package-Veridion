 ...
 */
export function openKycPopup(url, windowName) {
 ...
 */
export function isPopupOpen(popup) {
    if (!popup) {
        return false;
    }
    try {
 ...
 */
export function arePopupsBlocked() {
    try {
        // Try to open a test popup
        const testPopup = window.open("about:blank", "test-popup", "width=1,height=1");
        if (!testPopup) {
            return true;
        }
